...
export const RiskTimeline: React.FC<RiskTimelineProps> = ({
  ...
  return (
    <div className={`glass-panel p-5 bg-slate-900 border border-slate-800/80 text-slate-100 rounded-xl shadow-xl space-y-3 ${className || ''}`}>
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800 pb-3">
        <div>
          ...
          <p className="text-[11px] text-slate-400 mt-0.5">
            Duration: <span className="font-mono text-slate-200 font-bold">{maxDurationSec}s</span> • Peak Anomaly: <strong className="text-red-400">{String(peakPoint.time).padStart(2, '0')}s ({peakPoint.frameRisk || peakRisk}%)</strong>
          </p>
        </div>

        <div className="flex items-center gap-3">
          <div className="flex items-center gap-1.5 text-xs bg-slate-950 px-3 py-1 rounded-lg border border-slate-800">
            <TrendingUp className="w-3.5 h-3.5 text-red-400" />
            <span className="text-slate-400">Peak Risk:</span>
            <span className="font-mono font-bold text-red-400">{peakRisk.toFixed(1)}%</span>
          </div>

          <div className="flex items-center gap-1.5 text-xs bg-emerald-950/60 px-3 py-1 rounded-lg border border-emerald-800/80">
            <span className="text-emerald-300">Composite Index:</span>
      ...
      {activePoint?.event && activePoint.event !== 'Nominal Handling Stream' ? (
        <div className="bg-red-950/40 border border-red-500/40 p-2.5 rounded-xl text-xs flex items-center justify-between text-red-200 animate-pulse">
          <span className="font-semibold flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-red-500"></span>
            Anomaly Detected at {activePoint.time}s: {activePoint.event}
          </span>
          <span className="font-mono text-[10px] bg-red-900/60 px-2 py-0.5 rounded border border-red-500/50">
            R(t): {activePoint.frameRisk}%
          </span>
        </div>
      ) : (
        <div className="bg-slate-950 border border-slate-800/60 p-2 rounded-xl text-xs flex items-center justify-between text-slate-400">
          <span className="flex items-center gap-2">
            <span className="w-2 h-2 rounded-full bg-emerald-500"></span>
            Frame {roundedCurrentTime}s: Nominal Handling Stream (Risk R(t): {activePoint?.frameRisk || 15}%)
          </span>
          <span className="text-[10px] font-mono text-slate-500">YOLO11 Telemetry</span>
        </div>
      )}
      ...
      <div className="h-44 w-full">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis dataKey="timestamp" unit="s" stroke="#64748b" tick={{ fill: '#94a3b8', fontSize: 10 }} />
                ...
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="bg-slate-950 p-2.5 rounded-xl border border-slate-800 shadow-2xl text-xs font-mono space-y-1">
                      <p className="font-bold text-slate-200">Timecode: {data.timestamp}s</p>
                      <p className="text-red-400 font-bold">Risk Score R(t): {data.riskScore}%</p>
                      {data.event && <p className="text-amber-300 text-[11px] font-sans">{data.event}</p>}
                      <p className="text-slate-400 text-[10px]">Click point to seek video & auto-play</p>
                    </div>
                  );
                }
                return null;
      ...
      <div className="flex items-center justify-between text-[11px] text-slate-500 pt-1 border-t border-slate-800/80">
        ...
        <span className="font-mono text-slate-400">YOLO11 Kinematic Engine</span>
      </div>
    </div>
  );
};
