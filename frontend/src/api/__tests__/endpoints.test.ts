...
import { mockEvents, mockAnalytics, mockVideos } from '../mockData';
...
describe('Events API Service Layer (getEvents, getEventById, exportEventsCsv)', () => {
  ...
  it('getEvents falls back gracefully to filtered mockEvents when backend is offline', async () => {
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Network offline / connection refused'));

    const criticalEvents = await getEvents({ risk_level: 'Critical' });
    expect(criticalEvents.length).toBeGreaterThan(0);
    expect(criticalEvents.every(e => e.risk_level === 'Critical')).toBe(true);

    const searchedEvents = await getEvents({ search: 'drop' });
    expect(searchedEvents.length).toBeGreaterThan(0);
    expect(searchedEvents.every(e => 
      e.behaviour.toLowerCase().includes('drop') || 
  ...
  it('getEventById falls back to mockData when backend throws error', async () => {
    ...
    expect(event.behaviour).toBeDefined();
  });
  ...
  it('exportEventsCsv generates client-side CSV with UTF-8 BOM when backend is unreachable', async () => {
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Backend offline'));

    const blob = await exportEventsCsv();
    expect(blob).toBeInstanceOf(Blob);
    
    // Check UTF-8 BOM bytes in arrayBuffer
    ...
    // Check CSV text structure
    const text = await blob.text();
    expect(text).toContain('Event ID,Video ID,Timestamp (s)');
  });
});
...
describe('Analytics API Service Layer (getAnalyticsSummary, getBehaviourAnalytics, getRiskAnalytics, getTimelineAnalytics)', () => {
  ...
  it('getAnalyticsSummary queries /analytics/summary and handles fallback', async () => {
    ...
    const fallback = await getAnalyticsSummary();
    expect(fallback.summary.totalEvents).toBe(mockAnalytics.summary.totalEvents);
  });

  it('getBehaviourAnalytics queries /analytics/behaviours and handles fallback', async () => {
    ...
    expect(data[0].behaviour).toBe('Forklift Speeding');

    // Fallback test
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Failed to fetch'));
    const fallback = await getBehaviourAnalytics();
    expect(fallback.length).toBe(mockAnalytics.behaviours.length);
  });

  it('getRiskAnalytics queries /analytics/risk and handles fallback', async () => {
    const riskData = [
    ...
    // Fallback test
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Failed to fetch'));
    const fallback = await getRiskAnalytics();
    expect(fallback.length).toBe(4);
    expect(fallback.some(r => r.risk_level === 'Critical')).toBe(true);
  });

  it('getTimelineAnalytics queries /analytics/timeline and handles fallback', async () => {
    ...
    // Fallback test
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Failed to fetch'));
    const fallback = await getTimelineAnalytics();
...
describe('Assistant Chat API Service Layer (askAssistant)', () => {
  ...
  it('askAssistant falls back to local grounded reasoning when backend is offline', async () => {
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Backend unreachable'));

    const response = await askAssistant({ question: 'How can we prevent forklift collisions?' });
    expect(response.question).toBe('How can we prevent forklift collisions?');
    expect(response.answer).toBeDefined();
    expect(response.source_events).toBeInstanceOf(Array);
    expect(response.model_used).toBe('wms-intelligence-rag');
  });
});
...
describe('Videos API Service Layer (getVideos, getVideoById)', () => {
  ...
  it('getVideos queries /videos and returns video list', async () => {
    ...
    const fallback = await getVideos();
    expect(fallback.length).toBe(mockVideos.length);
  });
  ...
  it('getVideoById queries /videos/{videoId} and returns metadata', async () => {
    ...
    // Fallback test
    globalThis.fetch = vi.fn().mockRejectedValue(new Error('Network error'));
    const fallback = await getVideoById('nonexistent_id');
    expect(fallback).toBeDefined();
    expect(fallback.video_id).toBeDefined();
  });
});
